KeysDictionary('gen2', (1, 6),
               Key("text", String(help="text for humans")),
               Key("version", String(help="EUPS/git version")),
               Key("visit", Int(help="Gen2-assigned PFS visit")),
               Key("tel_focus",
                   String(name='name', help="Name of instrument focus position",
                          FITS=('FOC_POS', 'GEN2_FITS_PFS_FOC_POS')),
                   String(name='telpos', help="Focus where inst. is attached",
                          FITS=('TELFOCUS', 'GEN2_FITS_SBR_TELFOCUS')),
                   Float(name='val', units='um', help="Focus encoder value",
                         FITS=('FOC_VAL', 'GEN2_FITS_PFS_FOC-POS'))),
               Key("tel_axes",
                   Float(name='az', units='deg', help="Azimuth position",
                         FITS=('AZIMUTH', 'GEN2_FITS_SBR_AZIMUTH')),
                   Float(name='alt', units='deg', help="Altitude position",
                         FITS=('ALTITUDE', 'GEN2_FITS_SBR_ALTITUDE'))),
               Key("tel_rot",
                   String(name='name', help="Image rotator type",
                          FITS=('IMR_TYP', 'GEN2_FITS_SBR_IMR-TYP')),
                   Float(name='posAngle', units='deg', help="Inst pos angle at flange",
                         FITS=('INST-PA', 'GEN2_FITS_SBR_INST-PA')),
                   Float(name='instrot', units='deg', help="Inst rotation angle",
                         FITS=('INSROT', 'GEN2_FITS_SBR_INSROT')),
                   Float(name='imgrot', units='deg', help="Image rotation angle",
                         FITS=('IMGROT', 'GEN2_FITS_SBR_IMGROT'))),
               Key("tel_adc",
                   String(name='name', help="ADC name",
                          FITS=('ADC-TYP', 'GEN2_FITS_SBR_ADC-TYP')),
                   Float(name='angle', units='deg', help="ADC pos angle",
                         FITS=('ADC', 'GEN2_FITS_SBR_ADC'))),
               Key("dome_env",
                   Float(name='humidity', units='%', help="Dome humidity",
                         FITS=('DOM-HUM', 'GEN2_FITS_SBR_DOM-HUM')),
                   Float(name='pressure', units='hPa', help="Dome pressure",
                         FITS=('DOM-PRS', 'GEN2_FITS_SBR_DOM-PRS')),
                   Float(name='temperature', units='K', help="Dome temperature",
                         FITS=('DOM-TMP', 'GEN2_FITS_SBR_DOM-TMP')),
                   Float(name='wind', units='m/s', help="Dome wind speed",
                         FITS=('DOM-WND', 'GEN2_FITS_SBR_DOM-WND'))),
               Key("outside_env",
                   Float(name='humidity', units='%', help="Outside humidity",
                         FITS=('OUT-HUM', 'GEN2_FITS_SBR_OUT-HUM')),
                   Float(name='pressure', units='hPa', help="Outside pressure",
                         FITS=('OUT-PRS', 'GEN2_FITS_SBR_OUT-PRS')),
                   Float(name='temperature', units='K', help="Outside temperature",
                         FITS=('OUT-TMP', 'GEN2_FITS_SBR_OUT-TMP')),
                   Float(name='wind', units='m/s', help="Outside wind speed",
                         FITS=('OUT-WND', 'GEN2_FITS_SBR_OUT-WND'))),

               Key("header", String(help="Gen2-supplied FITS header")),
)
